...
# -*- coding: utf-8 -*-
"""
Build script for the A to Z Mysteries quiz site.

1. gen_quizzes()  -> reads _quizdata/<LETTER>.json and emits one self-contained
                     bilingual (EN/ZH) quiz HTML per book, following the
                     unwilling-umpire-quiz.html template exactly.
2. gen_hub()      -> rebuilds index.html listing ALL 26 books (A-Z):
                     A-T come from _quizdata JSON, U-Z are hardcoded (already live).
...
# Per-letter accent colors (U-Z reuse the colors already live on the site).
ACCENT = {
    "A": "#c0392b", "B": "#8e44ad", "C": "#2980b9", "D": "#16a085",
    "E": "#e67e22", "F": "#27ae60", "G": "#d4ac0d", "H": "#c2185b",
    "I": "#0097a7", "J": "#7e57c2", "K": "#388e3c", "L": "#f4511e",
    "M": "#5e35b1", "N": "#00897b", "O": "#fbc02d", "P": "#3949ab",
    "Q": "#d81b60", "R": "#43a047", "S": "#8d6e63", "T": "#1e88e5",
    "U": "#c0392b", "V": "#8e44ad", "W": "#2980b9", "X": "#16a085",
    "Y": "#e67e22", "Z": "#27ae60",
}
...
QUIZ_CSS = """  :root{
  ...
  *{box-sizing:border-box;}
  ...
  .baseball::before,.baseball::after{
    content:"";position:absolute;top:50%;width:2px;height:38px;
    background:#c0392b;transform:translateY(-50%) rotate(18deg);
    border-radius:2px;
  }
  .baseball::before{left:16px;}
  .baseball::after{right:16px;transform:translateY(-50%) rotate(-18deg);}
...
const L = {
  en: {
    subtitle: "Comprehension Quiz • 10 Questions • by Ron Roy",
    check: "Check My Answers",
    retry: "Try Again",
    langBtn: "🌐 中文",
    result: (s) => `You scored ${s} / 10`,
    perfect: "Perfect! You really read every page.",
    great: "Great job — you understood the mystery well.",
    ok: "Not bad — but a few clues slipped past you.",
    low: "Time for a re-read! The mystery is in the details.",
    ansLabel: "Answer"
  },
  zh: {
    subtitle: "阅读理解测验 • 共10题 • 作者：Ron Roy",
    check: "提交答案",
    retry: "再做一次",
    langBtn: "🌐 English",
    result: (s) => `你的得分：${s} / 10`,
...
def validate(d):
    errs = []
    if not d.get("letter") or not d.get("title"):
        errs.append("missing letter/title")
    qs = d.get("questions", [])
    if len(qs) != 10:
...
def gen_quizzes():
    ...
    for f in files:
        ...
        with open(path, encoding="utf-8") as fh:
            d = json.load(fh)
        errs = validate(d)
        letter = d.get("letter", f.replace(".json", ""))
        if errs:
...
<div class="wrap">
  <header>
    ...
    <p id="subtitle">Comprehension Quiz • 10 Questions • by Ron Roy</p>
  ...
  <footer>Based on <em>A to Z Mysteries: __TITLE__</em> (Random House, __YEAR__)</footer>
</div>
...
__JS__
        ...
        with open(out_path, "w", encoding="utf-8") as fh:
            fh.write(quiz_html)
        print(f"  [OK] {letter} -> {out_name}  (source: {d.get('source','?')})")
        manifest.append({
            "letter": letter,
            "title": title,
...
# U-Z are already live; hardcode their hub metadata here.
UZ = {
    "U": {"title": "The Unwilling Umpire", "slug": "unwilling-umpire",
          "emoji": "⚾", "desc": "A charity baseball game — and six signed baseballs go missing. ⚾",
          "file": "unwilling-umpire-quiz.html"},
    "V": {"title": "The Vampire's Vacation", "slug": "vampire-vacation",
          "emoji": "🧛", "desc": "A hotel guest says he's a vampire. Truth, or a trick? 🧛",
          "file": "vampire-vacation-quiz.html"},
    "W": {"title": "The White Wolf", "slug": "white-wolf",
          "emoji": "🐺", "desc": "A howling white wolf in the snowy woods. Real, or fake? 🐺",
          "file": "white-wolf-quiz.html"},
    "X": {"title": "The X'ed-Out X-Ray", "slug": "xed-out-x-ray",
          "emoji": "🩻", "desc": "A hospital x-ray is crossed out. Why hide the truth? 🩻",
          "file": "xed-out-x-ray-quiz.html"},
    "Y": {"title": "The Yellow Yacht", "slug": "yellow-yacht",
          "emoji": "⛵", "desc": "A shiny yellow yacht, a missing person, a clue that floats away. ⛵",
          "file": "yellow-yacht-quiz.html"},
    "Z": {"title": "The Zombie Zone", "slug": "zombie-zone",
          "emoji": "🧟", "desc": "Strange footsteps in the fog... is the swamp haunted? 🧟",
          "file": "zombie-zone-quiz.html"},
}


def gen_hub(manifest):
    # build a dict letter -> meta for all 26 books
    books = {}
    for m in manifest:
        books[m["letter"]] = m
    for L, m in UZ.items():
        if L not in books:
            books[L] = {
                "letter": L, "title": m["title"], "titleZh": "",
                "slug": m["slug"], "year": "", "emoji": m["emoji"],
                "desc": m["desc"], "source": "live", "file": m["file"],
            }
    ...
    for L in LETTER_ORDER:
        b = books.get(L)
        if not b:
            cards.append(f"    <!-- {L}: MISSING -->")
            continue
        accent = ACCENT.get(L, "#2e8b57")
        cards.append(f"""    <article class="book" style="--accent:{accent}">
      <div class="top"><div class="letter">{L}</div><div class="emoji">{b['emoji']}</div></div>
      <div class="body">
    ...
    count = len([c for c in cards if "MISSING" not in c])
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>A to Z Mysteries — Books</title>
<style>
  :root{{
    --grass:#2e8b57;
    --grass-dark:#1f6b41;
...
<header>
  <div class="topbar">
    <a class="homeBtn" href="../../index.html" title="Home / 首页" aria-label="Home">🏠</a>
    <h1>A to Z Mysteries</h1>
    <span style="width:40px"></span>
  </div>
  <p>初章书 · Early Chapter Books — by Ron Roy</p>
</header>

<div class="wrap">
  <div class="intro">
    🕵️ <b>{count} 个侦探小故事（A–Z），每本都有中英双语阅读理解测验。</b><br>
    {count} mysteries from A to Z, each with a bilingual (English / 中文) comprehension quiz. Tap a book, read the questions, then check your answers!
  </div>
...
if __name__ == "__main__":
    man = gen_quizzes()
    gen_hub(man)
    # also drop a manifest for reference
