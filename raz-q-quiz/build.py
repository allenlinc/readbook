#!/usr/bin/env python3
# Build the RAZ-Q quiz site from _quizdata/*.json
#   - raz-o-quiz/index.html        (hub: search + skill filter + grid)
#   - raz-o-quiz/books/<slug>.html  (one interactive page per book)
import json, os, glob, re, html

HERE = os.path.dirname(os.path.abspath(__file__))
DATA = os.path.join(HERE, "_quizdata")
BOOKS_DIR = os.path.join(HERE, "books")
SRC = os.path.join(HERE, "_sources.json")
...
if os.path.exists(SRC):
    for b in json.load(open(SRC, encoding="utf-8")):
        expected[b["slug"]] = b["title"]
...
# ---- report missing ----
have = {b["slug"]: b["title"] for b in books}
present_norm = {norm(t): t for t in have.values()}
not_done = [t for t in expected.values() if norm(t) not in present_norm]
if not_done:
    print("\n⚠️  MISSING (%d of %d expected):" % (len(not_done), len(expected)))
    for t in sorted(not_done): print("   -", t)
else:
    print("\n✅ All %d expected books present." % len(expected))
...
<title>RAZ-Q 阅读理解 · Reading by Level</title>
...
<div class="wrap">
  ...
  <div class="hub-hero">
    <h1><span class="en">RAZ Level Q · Reading Comprehension</span><span class="zh">RAZ Level Q · 阅读理解练习</span></h1>
...
__CARDS__
  ...
  <footer><span class="en">RAZ-Q Quick Check · questions written from the original books</span><span class="zh">RAZ-Q 阅读理解 · 题目依据原书内容编写</span></footer>
</div>
<script>
var BOOKS = __BOOKS__;
...
<div class="wrap">
  ...
  <div id="app"></div>
  <footer><a href="../index.html"><span class="en">← Back to RAZ-Q</span><span class="zh">← 返回 RAZ-Q 书单</span></a></footer>
</div>
